...
def schedule_view(request: Request, start: Optional[str] = Query(None)):
    ...
    try:
        # Ролловер (воскресенье 15:00 Europe/Berlin)
        ...
        if start:
            ...
            # Всегда окно 2 недели от текущего понедельника
            start_date = week_monday(today)
        ...
        # Можно ли генерировать следующую неделю (W+2)
...
async def schedule_begin_edit(request: Request, start_iso: str = Form(...)):
    """Ленивое начало редактирования: копируем текущее published окно → draft"""
...
def schedule_generate_next(request: Request, start_iso: str = Form(...)):
    """Сгенерировать РОВНО следующую неделю (W+2) как draft"""
    if not is_admin(request):
        return RedirectResponse(url="/schedule", status_code=302)
    ...
    next_week_start = start_date + timedelta(days=7)
    try:
        _, _dates = generate_schedule(next_week_start, weeks=1, persist=True)  # генератор пишет draft
        return RedirectResponse(url=f"/schedule?start={start_date.isoformat()}", status_code=302)
    except Exception:
...
async def schedule_save(
    ...
    """Публикация: собираем форму → published, затем синхронизируем идентичный draft"""
    if not is_admin(request):
        return RedirectResponse(url="/schedule", status_code=302)
    ...
    try:
        ...
        # Удаляем published и draft за окно — формируем заново
        db.execute(
            delete(Shift).where(
                Shift.date >= dates[0],
                Shift.date <= dates[-1],
